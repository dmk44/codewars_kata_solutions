# Remember
**DESCRIPTION**: Write a function that takes a string and returns an array of the repeated characters (letters, numbers, whitespace) in the string.
...
## Solution
```py
remember = lambda string: [char for i, char in enumerate(string) if char in string[:i] and string[:i].count(char) == 1]
```